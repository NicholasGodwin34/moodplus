...
# Ensure the project root is on sys.path so sibling packages (like `backend`)
# can be imported when running from inside `frontend/` or from repo root.
...
from backend.database import User, JournalEntry, get_session
